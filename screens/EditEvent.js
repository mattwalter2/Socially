...
export default function EditEvent( { navigation, route }) {
    ...
    const [eventBeingEditedInformation, setEventBeingEditedInformation] = useState({eventName: '', category: '', address: '', additionalNotes: '', description: '', private: null})
    ...
    useEffect(
        () => {
            ...
            const eventBeingEditedRef = doc(db, 'users', auth.currentUser.uid, 'posts', route.params.selectedEventCreatedByUser)
            onSnapshot(eventBeingEditedRef, (doc) => {
                setEventBeingEditedInformation(() => ({
            ...
            const getSelectedEventInformation = async () => {
                try {
                    const eventBeingEditedRef = doc(db, 'users', auth.currentUser.uid, 'posts', route.params.selectedEventCreatedByUser)
                    const eventBeingEdited = await getDoc(eventBeingEditedRef)
                    console.log(eventBeingEdited.data().address)
            ...
            getSelectedEventInformation()

    
        }
    , [])

    console.log(eventBeingEditedInformation)
    ...
    const handleUpdateEvent = async (eventname, category, starttime, endtime, address, additionalNotes, description) => {
        try { 
            const colRef = doc(db, 'users', auth.currentUser.uid, 'posts', route.params.selectedEventCreatedByUser)
  ...
  return (
    <SafeAreaView style={{flex: 1, margin: 20}}>
        ...
        <Formik
            ...
            initialValues={eventBeingEditedInformation}
            onSubmit={values => {
                const newEvent = handleUpdateEvent(values.eventname, values.category, startTime, endTime, values.address, values.additionalNotes, values.description)
